...
export const routes: Routes = [
  ...
  { path: 'profile', loadComponent: () => import('./components/profile/profile').then(m => m.Profile), canActivate: [authGuard], children: [
      { path: '', redirectTo: 'orders', pathMatch: 'full' },
      { path: 'orders', loadComponent: () => import('./components/profile/orders/orders').then(m => m.Orders) },
      { path: 'favorites', loadComponent: () => import('./components/profile/favorite/favorite').then(m => m.Favorite) },
      { path: 'about', loadComponent: () => import('./components/profile/aboutus/aboutus').then(m => m.Aboutus) },
    ]
  },
